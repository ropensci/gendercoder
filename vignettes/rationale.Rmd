...
sample %>% 
  head(10) %>% 
  mutate(recoded_gender = recode_gender(gender = Gender, dictionary = broad_en)) %>% 
  knitr::kable(caption = "The broad_en dictionary applied to `head(sample)`")
...
sample %>% 
  ...
  summarise(count = n()) %>% 
  arrange(-count) %>% 
  knitr::kable(caption = "Summary of gender categories after use of the *broad_en* dictionary")
...
using the `dictionary` argument. The first `dictionary = broad_en` provides 
...
categories by using the `narrow_en` dictionary (`dictionary = narrow_en`). This 
...
sample %>% 
  head(10) %>% 
  mutate(recoded_gender = recode_gender(gender = Gender, dictionary = narrow_en)) %>% 
...
supplement the inbuilt dictionary by providing a vector of vectors to the 
...
The defaults that we used are specific to our context and your data or context 
may be different. 

We offer two built-in dictionaries, broad and narrow. Both are necessarily 
...
In particular the narrow option makes opinionated choices about some responses 
that we want to acknowledge are potentially problematic. In particular,

- In 'narrow_en' coding intersex responses are recoded as 'sex and gender
diverse'
- In 'narrow_en' responses where people indicate they are trans and
...
this is more appropriate. We welcome people to update the inbuilt dictionary 
...
suitable for a broader range of cultural contexts. We particularly welcome the
addition of non-English dictionaries or of other genderdiverse responses to the 
broad_en and narrow_en dictionaries.
