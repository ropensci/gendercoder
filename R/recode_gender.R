...
#' recode_gender
#'
#' recode_gender matches uncleaned gender responses to cleaned list using an
#' inbuilt or custom dictionary.
#'
...
#' replacement values. A built-in dictionary example_dictionary is used by
...
recode_gender <-  function(gender = gender,
    ...
    if (exists(x.name, env) == FALSE) {
      stop(paste(x.name , "is not found"))
    }

    if (exists(dictionary.name, env) == FALSE) {
      stop(paste(dictionary.name, "is not found"))
    }
    ...
    recoded_list <- dictionary[tolower(gender)]
